...
const usersettings = () => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setUser(parsedUser);
      setUserData(parsedUser); // Inicializar userData con los datos actuales
  ...
  // Función para manejar el cambio en el campo de foto
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      const imageUrl = URL.createObjectURL(files[0]); // Crear un objeto URL tempora blob
      setUserData((prevData: any) => ({
        ...prevData,
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3003/users/updateUser/${user.id}`,
        {
          method: "PUT",
          headers: {
