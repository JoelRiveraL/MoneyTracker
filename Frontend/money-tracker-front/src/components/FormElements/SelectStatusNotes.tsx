...
const SelectStatusNote: React.FC<SelectStatusProps> = ({ id, onChange }) => {
  ...
  useEffect(() => {
    if (selected !== null) {
      onChange(options[selected].value); // Call onChange with selected value
    }
  }, [selected, options]); // Call onChange whenever selected changes
  ...
  const remove = () => {
    ...
    if (selected !== null) {
      ...
      setOptions(newOptions);
      setSelected(null);
      onChange(""); // Notify that no value is selected
    }
  };
                ...
                >
                  <div className="flex w-full flex-col">
                    {options.map((option, index) => (
                      <div key={index}>
                        <div
                          className="w-full cursor-pointer rounded-t border-b border-stroke bg-white p-2 pl-2 hover:bg-gray dark:border-strokedark dark:bg-form-input dark:hover:bg-stroke"
...
export default SelectStatusNote;
