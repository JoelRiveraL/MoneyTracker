    ...
    const fetchPayments = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Filtra los pagos para que solo se muestren aquellos cuyo tipo contenga la palabra "Deuda"
          const debts = data.filter((item: Payment) => item.type.toLowerCase().includes("deuda"));
          setPayments(debts);
        } else {
          console.error('Error fetching payments:', response.statusText);
          // Maneja el error, por ejemplo, redirigir al usuario al login
        }
      } catch (error) {
